...
test_that("output image type is array", {
  expect_true(is.array(shrink(image, 40, 40)))
})

test_that("new image dimension is correct", {
  expect_equal(dim(shrink(image, 20, 20))[1:2], c(20, 20))
  expect_equal(dim(shrink(image, 19, 19))[1:2], c(19, 19))
  expect_equal(dim(shrink(image, 15.0, 15.0))[1:2], c(15, 15))
  expect_equal(dim(shrink(image, 10, 10))[3], 3)
})

test_that("yield error when input type is invalid", {
  expect_error(shrink(image, 20.1, 20.1))
  expect_error(shrink(image_wrong_type, 10, 10))
})
